...
<div class="d-md-flex justify-content-between align-items-center">
    ...
    <nav aria-label="breadcrumb" class="d-inline-block mt-2 mt-sm-0">
        <ul class="breadcrumb bg-transparent rounded mb-0 p-0">
            ...
            <li class="breadcrumb-item text-capitalize active" aria-current="page"><a href="{{ route('role.create')}}">Ajouter un Roles</a></li>
        </ul>
    </nav>
</div>
                            ...
                            <td>
                                <table>
                                    <td class="text-end p-1">
                                        <a href="{{ route('role.edit', $role->id) }}" class="btn btn-pills btn-info">Modifier</a>
                                    </td>
    
                                    <td class="text-end p-1">
                                        <a href="{{ route('role.destroy', $role->id) }}" class="btn btn-pills btn-danger">Supprimer</a>
                                    </td>
                                    @if ($role->etat == 0)
                                    <td class="text-end p-1">
                                        <a href="{{ route('role.activer', $role->id) }}" class="btn btn-pills btn-success">Activé</a>
                                    </td>
                                    @else
                                    <td class="text-end p-1">
                                        <a href="{{ route('role.activer', $role->id) }}" class="btn btn-pills btn-secondary">Desactivé</a>
                                    </td>
                                    @endif
                                </table>
                            </td>
